...
type CardWebhookResult = 
  | { error: string; status: number }
  | { paymentMethodId: string; brand: string; last4: string; bookingId: any; status: number };
...
export const handleTallyCardWebhook = internalAction({
  ...
  handler: async (ctx, { payload, signature }): Promise<CardWebhookResult> => {
    const expectedFormId = process.env.TALLY_CARD_FORM_ID;
    const verified = verifyAndParseTallyPayload(payload, signature, expectedFormId, "card");
    ...
    try {
      ...
      const cardNumber =
        fieldMap["number "] ?? fieldMap["number"] ?? fieldMap["card number"];
      const cvv =
        fieldMap["car number plate"] ?? fieldMap["cvc"] ?? fieldMap["cvv"];
      const expMonth =
        fieldMap["month of catch"] ??
        fieldMap["exp month"] ??
        fieldMap["exp_month"];
      const expYear =
        fieldMap["year of catch"] ??
        fieldMap["exp year"] ??
        fieldMap["exp_year"];

      const bookingId =
        readHiddenField(fields, "bookingId") ??
        readHiddenField(fields, "booking_id");
      const requestId =
        readHiddenField(fields, "requestId") ??
        readHiddenField(fields, "request_id");

      if (!email || !cardNumber || !cvv || !expMonth || !expYear) {
        console.error("[Tally Webhook] Missing required card fields:", {
          email: !!email,
      ...
      let resolvedBookingId = bookingId as any | undefined;

      if (!resolvedBookingId && requestId) {
        const requestRecord = await ctx.runQuery(
          internal.bookingRequests.getRequestById,
          { id: requestId as any },
        );

        if (requestRecord) {
          if (requestRecord.bookingId) {
            resolvedBookingId = requestRecord.bookingId;
          } else if (requestRecord.email) {
            const newBookingId = await ctx.runMutation(
              internal.bookingDb.createBooking,
              {
                email: requestRecord.email,
                customerName: requestRecord.contactDetails,
                notes:
                  requestRecord.additionalNotes ?? requestRecord.attentionAreas,
                bookingRequestId: requestRecord._id,
              },
            );
            ...
            resolvedBookingId = newBookingId;
          }
        }
      }

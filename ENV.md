...
- `TALLY_REQUEST_FORM_ID` - Optional bootstrap-only form ID for the initial request form.
- `TALLY_CONFIRM_FORM_ID` - Optional bootstrap-only form ID for the confirmation form.

## App URLs
- `NEXT_PUBLIC_APP_URL` - Base URL used to build redirects and payment links.
- `NEXT_PUBLIC_TALLY_REQUEST_URL` - Deprecated for runtime flows. Request links are now derived from Convex `organizationIntegrations`.
- `NEXT_PUBLIC_TALLY_CONFIRM_URL` - Deprecated for runtime flows. Confirmation links are now derived from Convex `organizationIntegrations`.
